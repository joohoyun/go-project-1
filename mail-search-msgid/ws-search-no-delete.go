...
func main() {
	...
	getSetToken()
	fmt.Println("starting server... port 8080.... /delete func pass msgid and rcpt values in string format")
...
//func code1(w http.ResponseWriter, r *http.Request) {
...
func delete(w http.ResponseWriter, r *http.Request) {
	...
	if len(mappy1["value"].([]interface{})) == 0 {
		...
		p100 = mappy1["value"].([]interface{})[0].(map[string]interface{})["id"].(string)
		...
		fmt.Println(mappysummary) // NOT actually USED...

		mappy2["deleteStatusCode"] = statusCode

		json_data, _ := json.Marshal(mappy2)
		//body,_=ioutil.ReadAll(json_data)
		var prettyJSON bytes.Buffer
		err := json.Indent(&prettyJSON, json_data, "", "\t")
		if err != nil {
			fmt.Println(err)
		}
		string_data := string(prettyJSON.Bytes())

		fmt.Fprintf(w, string_data)

		//json.NewEncoder(w).Encode(mappy2) // mappy2 is the full output from the graph api call...
	}
...
func getSummary(id string) map[string]interface{} {
	...
	var custom_url = `https://graph.microsoft.com/v1.0/users/` + param_rcpt + `/messages/` + messageid
	client := &http.Client{}
	req, err := http.NewRequest("GET", custom_url, nil)
