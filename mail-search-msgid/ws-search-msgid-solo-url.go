...
func delete(w http.ResponseWriter, r *http.Request) {
	...
	if len(mappy1["value"].([]interface{})) == 0 {
		...
		mappy2 := mappy1["value"].([]interface{})[0].(map[string]interface{})
		...
		mappysummary["messageID"] = mappy2["internetMessageId"]
		mappysummary["isread"] = mappy2["isRead"]
		mappysummary["toRecipients"] = mappy2["toRecipients"]
		mappysummary["ccRecipients"] = mappy2["ccRecipients"]
		//mappysummary["sender"] = p_sender
		data := mappy2["body"].(map[string]interface{})["content"].(string)
		r, _ := regexp.Compile("http[s]?://[\\w./:]+") // worky
		//r, _ := regexp.Compile("^(https://urldefense).http[s]?://[\\w./:]+") // no worky
		fmt.Println("results.....")
		...
		json.NewEncoder(w).Encode(mappysummary)
